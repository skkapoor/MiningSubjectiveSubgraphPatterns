...
##################################################################################################################################################################
    def getSeeds(self):
        ...
        if 'all' in self.seedMode:
            ...
            Gid = ray.put(self.G)
            PDid = ray.put(self.PD)
            interestListids = None
            if self.gtype == 'U':
                interestListids = [computeInterestParallel.remote(LNit, Gid, PDid, self.mode, self.gtype, self.isSimple, self.incEdge, self.q) for LNit in ListNode]
            else:
                interestListids = [computeInterestParallelD.remote(LNit, Gid, PDid, self.mode, self.gtype, self.isSimple, self.incEdge, self.q) for LNit in ListNode]
            interestList = []
